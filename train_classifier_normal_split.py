...
all_folders = ['inhibitory', 'excitatory']
if use_subset:
    if subset == 'mg':
...
if retrain:
    ...
    reload_path = 'trials/2D_'+subset+'_trainvaltest_retrain=False_lr_0.0001_bs_4_epoch_130.pt'
...
if retrain:
    ...
    for t in type_to_cell.keys():
        ...
        for cell in cells:
            ...
            syn_inds = np.random.permutation(len(synapses))
            ind_1 = int(train_split*len(synapses))
            ind_2 = ind_1 + int(test_split*len(synapses))
            ind_3 = min(len(synapses), ind_2 + int(val_split*len(synapses)))
            train_inds = syn_inds[0:ind_1]
            test_inds = syn_inds[ind_1:ind_2]
            val_inds = syn_inds[ind_2:ind_3]
            for train_ind in train_inds:
                images = synapses[list(synapses.keys())[train_ind]]
                x_train += images
                for im in images:
            ...
            for val_ind in val_inds:
                images = synapses[list(synapses.keys())[val_ind]]
                x_val += images
                for im in images:
                    y_temp = [0]*len(all_folders)
                    y_temp[index] = 1
            ...
            for test_ind in test_inds:
                images = synapses[list(synapses.keys())[test_ind]]
                x_test += images
                for im in images:
                    y_temp = [0]*len(all_folders)
                    y_temp[index] = 1
